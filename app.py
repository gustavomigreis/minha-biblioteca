...
# IMPORTAÇÃO: Funções de processamento e sugestão
...
# NOTA: extract_file_id foi importada do pdf_processor
...
if menu == "Cadastro Automatizado (PDF)":
    ...
    if process_button:
        ...
        if raw_text and not raw_text.startswith("Erro"):
            ...
            if len(raw_text) > 100: # Verifica se o texto é significativo
                try:
                    # Chama a função de heurística de metadados (agora com TextRank)
    ...
    if st.session_state['extracted_text']:
        ...
        # Colunas para Formulário (1) e Preview (1)
        ...
        if deve_exibir_resultados and not df_unificado.empty:
            ...
            if id_selecionado:
                ...
                if recurso_tipo == 'B': # REFERÊNCIA BIBLIOGRÁFICA (PDF/DOCUMENTO)
                    ...
                    if caminho and str(caminho).startswith("http"):
                        # Reutiliza a função de extração do ID, que está importada
                        file_id = extract_file_id(caminho) 
                        if file_id:
                            preview_url = f"https://drive.google.com/file/d/{file_id}/preview"
                    ...
                    # Reutiliza a função de criação de link, que precisa ser definida aqui ou importada
