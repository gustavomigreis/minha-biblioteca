...
from data_collector import unified_data_search # Importa a função de busca online
...
# Inicializa o estado de visualização do layout e estados de coleta
...
if 'logs' not in st.session_state: # Estado para logs do PDF Processor
    st.session_state['logs'] = {}
if 'search_results_online' not in st.session_state: # Novo estado para resultados da coleta
...
if 'last_online_query' not in st.session_state:
    st.session_state['last_online_query'] = "mineração" # Valor padrão para a caixa de busca
...
if check_password():
    ...
    opcoes_menu.extend([
        "Coleta de Dados Online", # NOVO ITEM AQUI
...
# Tenta carregar o menu, se houver um redirecionamento pendente
if 'menu_selection' in st.session_state:
    # Força a seleção do menu para o item transferido
...
# =========================================================================
# === PÁGINA: COLETA DE DADOS ONLINE (NOVA PLATAFORMA INTEGRADA) ===========
# =========================================================================

if menu == "Coleta de Dados Online":
    ...
    st.markdown(f"""
        Busque recursos acadêmicos e relatórios econômicos com foco em **Formação Socioespacial do Peru**.
        *Atenção: Esta busca utiliza raspagem (scraping) em fontes públicas (ex: Google Scholar) e pode ser instável.*
    """)

    # Formulário de Busca
    search_query = st.text_input(
        "Termo de Busca Focado (Ex: mineração, desigualdade urbana)",
        # Usa o último termo de busca ou o padrão
        ...
        if search_query:
            ...
            with st.spinner(f"Buscando por '{search_query}' nas fontes online..."):
                try:
                    ...
                    # Verifica se houve algum erro de bloqueio/conexão
                    if any(res.get('tipo') == 'Erro' for res in results):
                         st.warning("Busca concluída, mas houve erros de conexão/bloqueio em algumas fontes.")
                    else:
                         st.success(f"Busca concluída. {len(results)} resultados encontrados.")
                         
                except Exception as e:
                    st.error(f"Erro inesperado durante a busca: {e}. Verifique a importação do `data_collector.py`.")
        else:
            st.warning("Por favor, insira um termo de busca.")

    # --- Exibição e Seleção de Resultados ---
    ...
    if results:
        ...
        # Cria um DataFrame para exibir com AgGrid
        ...
        # Correção do ValueError: Usa .get('selected_rows') e verifica se não está vazio
        selected_rows_df = grid_response.get('selected_rows') 
        
        selected_item = None
        if selected_rows_df is not None and not selected_rows_df.empty: # CORREÇÃO AQUI
        ...
        # --- Painel de Detalhes e Ações (Integração) ---
        if st.session_state['selected_online_item']:
            ...
            # Opção de AÇÃO: Enviar para Cadastro Automatizado
            if item['link'] != '#':
                ...
                # Botão que transfere o link e muda a página
...
# =========================================================================
    ...
    if link_transferido:
        st.warning(f"Link recebido da Coleta Online: **{title_transferido}**. Clique em 'Processar' para extrair os metadados.")
    
    # Se o link não veio do transfer, usa o valor que já estava na caixa de texto
    ...
    if process_button:
        ...
        if raw_text and not raw_text.startswith("Erro"):
            ...
            # --- INTEGRAÇÃO DA FUNÇÃO DE SUGESTÃO DE METADADOS (Recebe logs) ---
            ...
            if logs:
                 with st.expander("Ver Logs e Status de Execução da IA"):
                     for key, value in logs.items():
                         ...
                         if 'Sucesso' in value:
                             status_emoji = "✅ "
                         elif 'Falhou' in value or 'Erro' in value:
                             status_emoji = "❌ "
                         elif 'Aviso' in value:
                             status_emoji = "⚠️ "
